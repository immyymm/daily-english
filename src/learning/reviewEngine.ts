...
export function statusFor(progress: CardProgress, score: number): MasteryStatus {
  if (progress.wrongCount + (score < 60 ? 1 : 0) >= 3) return '薄弱词';
...
export function applyReviewScore(progress: CardProgress, score: number, now = new Date()): CardProgress {
  ...
  const unstable = score >= 60 && score < 75;
  ...
  const correctStreak = passed ? progress.correctStreak + 1 : 0;
  const weak = wrongCount >= 3 ? true : progress.weak && correctStreak < 2;
  const temporary = { ...progress, wrongCount, correctStreak, weak };

  return {
    ...temporary,
    stage: nextStage,
    nextReviewAt: nextDueDate(progress.stage, now, score).toISOString(),
