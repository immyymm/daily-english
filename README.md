...
- 30 天、150 张离线完整词卡，全部选自项目附带的 COCA 词表；内容一次性预生成，网页打开词卡时不调用词典或生成 API
- 学习顺序固定为“主词性：动词 → 名词 → 形容词 → 副词 → 其他词性”，同一词性再按该词性的 COCA 名次排序；罕见的第二词性不会把名词误排进动词组
- 150 张词卡全部通过锁定模板、COCA 录入、主词性、词频顺序、发音、真实例句、题目语境和重复项校验；APP 不展示内部生产标签
- 每日独立词卡文件与全部词卡汇总文件
...
生产构建位于 `dist/`。构建命令会校验锁定模板与示例的 SHA-256、COCA 覆盖、主词性分组、组内词频顺序、稳定卡片 ID、重复内容、模板套话、关系词词性、真实例句、题目语境、日期和每日卡片数。`work` 继续精确保持锁定示例规模；其余词卡不为追求数量填入生僻搭配，但必须满足完整释义、核心结构、真实语境、固定搭配、语义对比、可用词族、真实例句和不少于 15 道词卡来源题目的完整性门槛。关系词和词组的北美英语发音数据来自 CMU Pronouncing Dictionary，并在内容构建阶段保留重音并转为 IPA；网页运行时不会调用词典 API。
