...
生产构建位于 `dist/`。构建命令会校验锁定模板与示例的 SHA-256、COCA 覆盖、主词性分组、组内词频顺序、稳定卡片 ID、重复内容、模板套话、关系词词性、双语释义和例句、题目语境、日期和每日卡片数。`work` 继续精确保持锁定示例规模；其余词卡不为追求数量填入生僻搭配，但必须满足不少于 2 条双语义项、3 个核心结构、6 个来自完整例句的真实语境、6 个含双语例句的固定搭配、逐条语义辨析、可用词族与不少于 15 道词卡来源题目的完整性门槛。近反义、词族与语义关联由 Princeton WordNet 与 ECDICT 在构建阶段离线辅助筛选，关系词和词组的北美英语发音数据来自 CMU Pronouncing Dictionary；网页运行时不会调用词典 API。
