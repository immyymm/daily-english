-- An INSERT ... ON CONFLICT DO UPDATE still fires a statement-level INSERT
-- trigger. The client refreshes its profile on every connection, so the old
-- statement trigger regenerated a daily plan immediately after a reset and on
-- every deployment reconnect. A row trigger fires only for a genuinely new
-- profile.
drop trigger if exists daily_english_profile_generate_initial_plan
on public.daily_english_profiles;

create trigger daily_english_profile_generate_initial_plan
after insert on public.daily_english_profiles
for each row execute function private.daily_english_generate_plan_after_profile_insert();
...
begin
  ...
  -- Ensure the profile exists before clearing. If this is the first insert,
  -- its initial-plan trigger runs now and that generated plan is deleted below.
  update public.daily_english_profiles
  set first_study_date = v_first_study_date,
...
revoke all on function public.daily_english_clear_my_learning_data()
from public, anon, authenticated;
grant execute on function public.daily_english_clear_my_learning_data()
to authenticated;

comment on function public.daily_english_clear_my_learning_data() is
  'Atomically clears the authenticated user''s Daily English history without regenerating a plan during the reset.';
